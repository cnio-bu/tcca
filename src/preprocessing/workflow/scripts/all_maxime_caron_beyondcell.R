...
filter_malignants <- function(sc) {
    subtypes_to_keep <- c("Malignant cells")
    if (sum(sc@meta.data$Celltype..malignancy. %in% subtypes_to_keep) > 0) {
        sc_filtered <- subset(x = sc, subset = Celltype..malignancy. %in% subtypes_to_keep)
        return(sc_filtered)
    } else {
        return(NULL)
    }
