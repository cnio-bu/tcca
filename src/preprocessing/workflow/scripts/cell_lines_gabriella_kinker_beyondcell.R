...
malignants <- seu[sapply(seu, ncol) > 100]
...
single_cell_report <- data.frame(
    sample = sapply(seu, FUN = function(x){unique(x@meta.data$Cell_line)}),
    cells = sapply(seu, FUN = function(x){ nrow(x@meta.data)})
)

bc_report <- data.frame(
    sample = sapply(malignants, FUN = function(x){ unique(x@meta.data$Cell_line)}),
