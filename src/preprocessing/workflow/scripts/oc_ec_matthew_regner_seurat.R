...
fill_metadata <- function(sc) {
  ...
  cells_to_keep <- intersect(colnames(sc), meta_set$cell_barcode)
  ...
  meta_set <- meta_set %>%
    filter(cell_barcode %in% cells_to_keep) %>%
    as.data.frame()
  rownames(meta_set) <- meta_set$cell_barcode
  meta_set$cell_barcode <- NULL
