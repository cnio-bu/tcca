...
meta_data <- read.delim(metadata, sep = ",", row.names = 1)
...
sample_list <- Seurat::SplitObject(object = sc, split.by = "orig.ident")
names(sample_list) <- sapply(sample_list, function(sc){unique(sc$"orig.ident")})

# Filter cells
samples_filtered <- lapply(sample_list, filter_sc)
...
samples_filtered <- lapply(samples_filtered, rename_columns, 
                              malignancy_colname = "celltype_major", 
                              malignant_names = c("Cancer Epithelial"),
                              cell_type_colname = "celltype_major",
