...
malignant_sc <- readRDS(
    paste0(data_folder, "/obj/urothelial_chen/all_malignant.rds")
)
...
## If any sample has <= 300 sigs. we'll have to remove it
samples_to_keep <- report[report$sigs >= 300, "sample"]
...
saveRDS(
    object = bcs_filtered,
    file = paste0(save_to, "/obj/urothelial_chen/bcs.rds")
)

write.table(
    x = report[samples_to_keep, ],
    file = paste0(save_to, "/obj/urothelial_chen/report.tsv"),
    sep = "\t",
    row.names = FALSE
)
