...
malignant_sc <- readRDS(
    paste0(data_folder, "/obj/luad_kim_nayoung/all_malignant.rds")
)
...
saveRDS(
    object = bcs_filtered,
    file = paste0(save_to, "/obj/luad_kim_nayoung/bcs.rds")
)

write.table(
    x = report[samples_to_keep, ],
    file = paste0(save_to, "/obj/luad_kim_nayoung/report.tsv"),
    sep = "\t",
    row.names = FALSE
)
