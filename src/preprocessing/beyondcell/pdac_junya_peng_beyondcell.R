...
malignant_sc <- readRDS(
    paste0(data_folder, "/obj/pdac_junya_peng/all_malignant.rds")
)
...
# Name the list 
names(malignant_sc) <- sapply(malignant_sc, function(sc){unique(sc$orig.ident)})
...
## If any sample has <= 200 sigs. we'll have to remove it
samples_to_keep <- report[report$sigs >= 200, "sample"]
...
saveRDS(
    object = bcs_filtered,
    file = paste0(save_to, "/obj/pdac_junya_peng/bcs.rds")
)

write.table(
    x = report[samples_to_keep, ],
    file = paste0(save_to, "/obj/pdac_junya_peng/report.tsv"),
    sep = "\t",
    row.names = FALSE
)
