...
malignant_sc <- readRDS(
    paste0(data_folder, "/obj/pancancer_dalia_barkley/all_malignant.rds")
)
...
saveRDS(
    object = bcs_filtered,
    file = paste0(save_to, "/obj/pancancer_dalia_barkley/bcs.rds")
    )

write.table(
    x = report[samples_to_keep, ],
    file = paste0(save_to, "/obj/pancancer_dalia_barkley/report.tsv"),
    sep = "\t",
    row.names = FALSE
    )
