...
calculate_bc_scores <- function(sc) {
    ...
    bc <- bcRegressOut(bc = bc, vars.to.regress = c("nFeature_RNA", "G2M.Score")) ## no G2M.Score since we could not estimate it
    return(bc)
}
...
malignant_sc <- readRDS(
    paste0(data_folder, "/obj/adrenalnb_rui_chong/all_malignant.rds")
)
...
malignant_sc <- malignant_sc[sapply(malignant_sc, ncol) > 300]
...
saveRDS(
    object = bcs_filtered,
    file = paste0(save_to, "/obj/adrenalnb_rui_chong/bcs.rds")
)

write.table(
    x = report[samples_to_keep, ],
    file = paste0(save_to, "/obj/adrenalnb_rui_chong/report.tsv"),
    sep = "\t",
    row.names = FALSE
)