...
malignant_sc <- readRDS(
    paste0(data_folder, "/obj/luad_philip_bisschof/all_malignant.rds")
)
...
saveRDS(
    object = bcs_filtered,
    file = paste0(save_to, "/obj/luad_philip_bisschof/bcs.rds")
)

write.table(
    x = report[samples_to_keep, ],
    file = paste0(save_to, "/obj/luad_philip_bisschof/report.tsv"),
    sep = "\t",
    row.names = FALSE
)
