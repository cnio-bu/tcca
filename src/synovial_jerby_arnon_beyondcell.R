...
malignant_sc <- readRDS(
    paste0(data_folder, "/obj/synovial_jerby_arnon/all_malignant.rds")
)
...
saveRDS(
    object = bcs_filtered,
    file = paste0(save_to, "/obj/synovial_jerby_arnon/bcs.rds")
)

write.table(
    x = report[samples_to_keep, ],
    file = paste0(save_to, "/obj/synovial_jerby_arnon/report.tsv"),
    sep = "\t",
    row.names = FALSE
)
