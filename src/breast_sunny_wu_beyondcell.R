...
malignant_sc <- readRDS(
    paste0(data_folder, "/obj/breast_sunny_wu/all_malignant.rds")
)
...
saveRDS(
    object = bcs_filtered,
    file = paste0(save_to, "/obj/breast_sunny_wu/bcs.rds")
)

write.table(
    x = report[samples_to_keep, ],
    file = paste0(save_to, "/obj/breast_sunny_wu/report.tsv"),
    sep = "\t",
    row.names = FALSE
)
