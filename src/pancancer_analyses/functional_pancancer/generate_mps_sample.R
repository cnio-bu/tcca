...
# top most expressed genes in each sample. We transform this list of matrices into 
# a list where each element corrresponds to all the NMF gene-scores of a single 
# sample (X genes x 39 programs).
...
# Load NMF matrices
setwd("/storage/scratch01/shared/projects/bc-meta/functional_nmf/sample_wise")

nmf_programs <- c()
for (study in list.files("./nmf_study")) {
...
# Get a list where each entry is a matrix with x rows (genes) x 39 columns
# (NMF programs)
...
print("Intersection between robust programs computed")
...
sample_nmf <- readRDS("metaprograms_cpm/sample_nmf.rds")
...
while (sorted_intersection[1] > min_group_size) {
  ...
  # frequency). When the number of genes with a given frequency span bewond the
...
saveRDS(cluster_list, "metaprograms_cpm/cluster_list15.rds")
saveRDS(mp_list, "metaprograms_cpm/mp_list15.rds")

cluster_list <- readRDS("metaprograms_cpm/cluster_list.rds")
mp_list <- readRDS("metaprograms_cpm/mp_list.rds")

# Transform to dataframe
max_len <- max(sapply(cluster_list, length))
cluster_list_padded <- lapply(cluster_list, function(x) { length(x) <- max_len; x })
cluster_df <- as.data.frame(cluster_list_padded, stringsAsFactors = FALSE)
write.table(cluster_df, "mps_nmfprograms.tsv", row.names = FALSE)

mp_df <- do.call(cbind, mp_list)
write.table(mp_df, "mps_genes.tsv", row.names = FALSE)
------------------------------------------------------------------------------
# Heatmap of Jaccard similaity scores for NMF programs clustered into MPs
# ------------------------------------------------------------------------------
# Remove MPs with high number of mitochodrial or ribosomal genes, coming from
# NMF programs only from one study or composed of less than 10 NMF programs
cancer_states <- c("MP1", "MP3", "MP29", "MP13", "MP33", "MP51", "MP7", "MP2", 
                   "MP16", "MP6", "MP28", "MP35", "MP36", "MP5", "MP14", "MP34", 
                   "MP12", "MP15", "MP20", "MP47", "MP49", "MP8", "MP37", "MP41",
                   "MP4", "MP11", "MP31", "MP57", "MP9", "MP50", "MP30") 
         
lineage_specific <- c("MP10", "MP19","MP22", "MP26", "MP32", "MP39", "MP17", 
                      "MP24", "MP27", "MP42", "MP56", "MP25")

mps <- c(cancer_states, lineage_specific)
cluster_list <- cluster_list[mps]
mp_list <- mp_list[mps]
names(mp_list) <- paste0("MP", 1:43)
names(cluster_list) <- paste0("MP", 1:43)
...
sex_inferred <- seu@meta.data %>%
  mutate(study_sample = paste0(study, ".", sample)) %>%
...
breaks <- c(
  ...
  seq(4.0001, 15, length.out = n_magma)
)
...
heat <- ComplexHeatmap::Heatmap(
  ...
  row_split = cluster_annotation$Metaprogram,
  cluster_columns = FALSE,
  cluster_column_slices = TRUE,
  show_column_dend = FALSE,
  column_split = cluster_annotation$Metaprogram,
...
png(
  file = "heatmap_mps_recurrent.png",
  res = 500,
  width = 10,
  height = 10,
  units = "in"
...
# Save the reordered metaprograms
saveRDS(mp_list[mps], "metaprograms_cpm/mp_list_reordered.rds")
...
universe <- unique(unlist(lapply(sample_nmf, rownames)))

# Define the MSigDB categories you want
msig_categories <- c("H", "C2", "C4", "C5", "C8")

# Create a named list of gene sets per category
msig_lists <- lapply(msig_categories, function(cat) {
  df <- msig_df %>% filter(gs_cat == cat)
  split(df$gene_symbol, df$gs_name)
})
names(msig_lists) <- paste0("msig_", msig_categories)

# Load custom gene sets
gmt_df <- read.gmt("/home/lmgonzalezb/Documents/bc-meta/bc-meta_repo/bc-meta/reference/combined_gsets_functional.gmt")
custom_gmt_list <- split(gmt_df$gene, gmt_df$term)

# Function to run ORA
run_ora <- function(gene_sets, mp_list, universe, source_tag) {
  lapply(names(mp_list), function(mp_name) {
    res <- fgsea::fora(pathways = gene_sets,
...
# Run ORA separately per collection
ora_h <- run_ora(msig_lists[["msig_H"]], mp_list, universe, source_tag = "MSigDB_H")
ora_c2 <- run_ora(msig_lists[["msig_C2"]], mp_list, universe, source_tag = "MSigDB_C2")
ora_c4 <- run_ora(msig_lists[["msig_C4"]], mp_list, universe, source_tag = "MSigDB_C4")
ora_c5 <- run_ora(msig_lists[["msig_C5"]], mp_list, universe, source_tag = "MSigDB_C5")
ora_c8 <- run_ora(msig_lists[["msig_C8"]], mp_list, universe, source_tag = "MSigDB_C8")
ora_custom <- run_ora(custom_gmt_list, mp_list, universe, source_tag = "Custom")


# Combine all results
all_ora <- bind_rows(ora_h, ora_c2, ora_c4, ora_c5, ora_c8, ora_custom)

# Optional: Save full result table for exploration
write.xlsx(all_ora, "all_ora_results_full121010.xlsx", overwrite = TRUE)

# Create ranked wide-format table
ranked <- all_ora %>%
  group_by(MP) %>%
  arrange(pval, desc(overlap), .by_group = TRUE) %>%
  mutate(Rank = row_number(),
         Pathway_Label = paste0(pathway, " [", Source, "]")) %>%
  select(MP, Rank, Pathway_Label)

wide_table <- ranked %>%
  pivot_wider(names_from = MP, values_from = Pathway_Label) %>%
  select(-Rank)
wide_table <- wide_table[paste0("MP", 1:length(mp_list))]

# Save wide-format ranked pathway table
write.xlsx(wide_table, "metaprogram_functional_annotation121010.xlsx", overwrite = TRUE)
...
saveWorkbook(wb, file = "fgsea_c8_ora15.xlsx", overwrite = TRUE)
...
# Save the workbook to a file
saveWorkbook(wb, file = "tmp/fgsea_custom_gsets_ora13_10.xlsx", overwrite = TRUE)
...
# Save the workbook to a file
saveWorkbook(wb, file = "fgsea_var_gsets_ora15.xlsx", overwrite = TRUE)
...
##### PLOT RELEVANT GENES FROM EACH CLUSTER ####
# Create a dataframe with all NMF programs from each subclone and k
all_genes <- Reduce(union, lapply(sample_nmf, function(df) rownames(df)))
nmf_programs_list <- lapply(sample_nmf, function(df) {
  genes_out <- setdiff(all_genes, rownames(df))
...
nmf_programs_list <- lapply(nmf_programs_list, function(df) df[order(rownames(df)), ])
names(nmf_programs_list) <- NULL
nmf_programs_mat <- do.call(cbind, nmf_programs_list)

# Select NMF programs and genes of interest
genes <- unique(unlist(mp_list))
names(genes) <- NULL
programs <- unlist(cluster_list)
names(programs) <- NULL
nmf_programs_mat <- nmf_programs_mat[genes, programs]

# Compute log2FC per sample/metaprogram
epsilon <- 1e-6
programs <- colnames(nmf_programs_mat)

log2fc_mat <- sapply(programs, function(p) {
  in_prog <- nmf_programs_mat[, p] + epsilon
  out_prog <- rowMeans(nmf_programs_mat[, setdiff(programs, p)]) + epsilon
  log2(in_prog / out_prog)
})

col_fun <- colorRamp2(c(0, 0.5, 2), c("lightblue", "white", "darkred"))
heat_split <- rep(names(cluster_list), lengths(cluster_list))
heat_genes <- ComplexHeatmap::Heatmap(
  mat = log2fc_mat,
  col = col_fun,
  cluster_rows = FALSE,
  ...
  show_column_dend = FALSE,
  column_split = factor(heat_split, levels = paste0("MP", 1:43)),
  clustering_distance_columns = "pearson",
  clustering_distance_rows = "pearson",
