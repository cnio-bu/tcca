...
seu.lvl1 <- readRDS(
  "/storage/scratch01/shared/projects/bc-meta/single_cell/seurat/v5/lvl1/seu_lvl1.rds"
)
...
doi <- unique(c(
  intersect(onTarget$drugCategory$name, ssc_bc),
  names(approved_drugs_model)
))

# What features are we using?
genesUsed_toBuild <- Reduce(intersect, list(
...
features_bulk_all_cancer_type_Cavg <- run_parallel_feature_ranking_bulk(
...
na_drug_models <- sapply(features_bulk_all_cancer_type_Cavg, function(x)
  (length(x) == 1 && is.na(x)))
...
for (i in 1:length(doi)) {
  ...
  print(sapply(raw_models_output, function(x)
    x$performance_in_scRNA))
  performance_by_features[[i]] <- lapply(raw_models_output, function(x)
    x$performance_in_scRNA)
  
  # Store the Tuned model
  for_output_tcca[[i]] <- err_handle(raw_models_output[[which.max(sapply(raw_models_output, function(x)
...
na_drug_models <- sapply(for_output_tcca, function(x)
  (length(x) == 1 && is.na(x)))
for_output_tcca <- for_output_tcca[!na_drug_models]
tuned_models_output <- for_output_tcca
saveRDS(
  tuned_models_output,
  paste('./Data/PERCEPTION_models_doi_tcca', Sys.time(), '.RDS', sep = '')
)
...
saveRDS(
  significant_drugs_names,
  "/home/mgonzalezb/bc-meta/PERCEPTION/Data/significant_drugs.Rds"
)
...
for (study_name in unique(seu.lvl1$study)) {
  ...
  write_matrix_dir(
    mat = killing_mat,
    dir = paste0(
      "/storage/scratch01/users/mgonzalezb/bc-meta/perception/",
      study_name
    ),
    overwrite = TRUE
  )
