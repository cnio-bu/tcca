...
modules_annotated_clinical <- read_tsv(file = "results/annotation/modules_annotated_clinical.tsv")
...
## next module: Patient primary treated
## Perform aggregation of metagroup 1
modules_mt2 <- modules_annotated_clinical %>%
    filter(metagroup == "patient_primary_treated")

cancers_to_consider <- modules_mt2 %>%
    group_by(tumor_type) %>%
    select(sample) %>%
    summarise(
        n.samples = length(unique(sample))
...
for (cancer in cancers_to_consider) {
    ...
    modules_mt_by_cancer <- modules_mt2 %>%
        filter(
            tumor_type == cancer
        ) %>%
        mutate(
    ...
    ggsave(
        plot = dist_plot_2,
        filename = paste0("results/modules/annotated/", cancer, "_treated_info_content.png"),
        height = 10,
        width = 10,
        dpi = 300
    )
