...
all_modules_treated <- list.files(
    ...
    pattern = "patient_primary_untreated_.*_communities\\.tsv",
    full.names = TRUE
)
...
write.table(
    x = comms,
    file = paste0("results/modules/annotated/patients_primary_untreated_meta_groups.tsv"),
    sep = "\t",
    row.names = FALSE
)
...
ggsave(
    plot = dplot,
    filename = "results/figures/proportion_drugset_untreated_dist.png",
    dpi = 100,
    height = 10,
    width = 10
)
...
write.table(
    x = drugs_com,
    file = "results/modules/annotated/metagroup_patients_untreated_consensus_drugs.tsv"
)
