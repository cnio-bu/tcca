...
robust_nmf_programs <- function(nmf_programs, intra_min = 70, intra_max = 20, inter_filter=T, inter_min = 20) {
  ...
  # clusterExport(cl, varlist = c("nmf_sel_unlist"))
  inter_intersect <- parApply(cl = cl, nmf_sel_unlist , 2, 
                              function(x) apply(nmf_sel_unlist , 2, function(y) length(intersect(x,y)))) ## calculating intersection between all programs
  stopCluster(cl)
