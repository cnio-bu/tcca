...
clonality <- read.table("../../cna_metadata/full_clonality_table_lvl2.tsv",
                        row.names = NULL)
...
metadata <- left_join(metadata, clonality, by = c("cell" = "original_barcode")) %>%
  mutate(
    scevan_subclone = ifelse(
      scevan_subclone != "non_tumor",
      paste(study, sample, scevan_subclone, sep = "."),
      scevan_subclone
    ),
...
clinical <- read.table("../v5/clinical_metadata_v4_new.tsv",
                       header = TRUE,
                       sep = "\t")
...
metadata_hugo <- metadata_hugo %>%
  mutate(across(all_of(cols_to_replace), ~ coalesce(get(
    paste0(cur_column(), ".new")
  ), .))) %>%
...
metadata <- metadata %>%
  mutate(tumor_type = ifelse(
    grepl("astrocitoma|astrocytoma", tumor_subtype),
    "A",
    tumor_type
  ))
