...
pc <- open_matrix_dir(dir = "v5/pdac_shu_zhang")
...
# Select common drugs
# drugs_bc <- readRDS("/storage/scratch01/users/mgonzalezb/bc-meta/studywise_analyses/perception_newmodels/drug_models/drugs_bc.rds")
...
ggsave(
  filename = "perception_clustering/all_drugs/loadings_lose_heats_33.png",
  heats,
  width = 29,
  height = 16,
  dpi = 300
...
ggsave(
  filename = "perception_clustering/all_drugs/elbow_plot_seu.png",
  elbow,
  width = 16,
  height = 9,
  dpi = 100
...
ggsave(
  "perception_clustering/all_drugs/clustree.png",
  clustree_tc,
  width = 8,
  height = 12
)
...
for (i in seq(0.1, 1, 0.1)) {
  ...
  ggsave(
    paste0("perception_clustering/all_drugs/umap_clusters", i, ".png"),
    dimplot_clusters,
    width = 10,
    height = 8
  )
...
saveRDS(seu,
        "perception_clustering/all_drugs/seu_pc_clustering.rds")
...
top_annot_df <- seu@meta.data[colnames(scaled_mat), c("sample_type", "patient", "beyondcell_snn_res.0.3")]
colnames(top_annot_df) <- c("Sample type", "Patient", "Beyondcell clusters")
pals <- list(
  "Sample type" = c("m" = "#C10044", "p" = "#F0BFD0"),
  "Patient" = c(
    "P01" = "#F8766D",
    "P02" = "#7CAE00",
    "P03" = "#00BFC4",
...
drugs <- data.table::fread("~/bc-meta/reference/final_moas - Collapsed.tsv") %>%
  ...
  mutate(
    collapsed.MoAs = case_when(
      ...
      !(preferred.drug.names %in% toupper(rownames(mat))) ~ "Other",
      TRUE ~ collapsed.MoAs
    )
  ) %>%
  distinct() %>%
...
png(
  file = "perception_clustering/new_models/common_drugs/heatmap_perception_clusters.png",
  res = 500,
  width = 19,
  height = 14,
  units = "in"
