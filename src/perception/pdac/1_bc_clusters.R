...
bc <- open_matrix_dir(dir = "./beyondcell/studywise_bpcells/pdac_shu_zhang")
bc <- as.matrix(bc)
bc[is.na(bc)] <- 0
...
heat <- ComplexHeatmap::Heatmap(
  ...
  row_labels = paste0(
    drugs[rownames(scaled_mat), "preferred.drug.names"]),
    show_row_names = TRUE,
    column_names_rot = 45,
...
pc <- readRDS(
"perception_clustering/new_models/common_drugs/seu_pc_clustering.rds"
)
df <- data.frame(
"beyondcell_clusters" = bc$beyondcell_snn_res.0.1,
"perception_clusters" = pc$RNA_snn_res.0.1
)
write.table(df, "pdac_clusters_common_drugs.tsv")